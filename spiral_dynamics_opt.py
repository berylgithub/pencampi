...
def spiral_dynamics_optimization_mpi(F, S, R, m, theta, r, kmax, domain, *f_args, log=True):
    '''
    Function F optimization using spiral dynamics -> rotating & contracting points
    '''
...
#        # rotate the points
    ...
    x_star = comm.bcast(x_star, root=0)
    F_xstar = comm.bcast(F_xstar, root=0)
...
#        print("time = ",time.time()-start)
    ...
    def unit_tests_multi_cores():
        ...
        ### temporary solution just let all cores return identical values copied from core 0
        xstar, F_xstar = spiral_dynamics_optimization_mpi(F, transformation_matrix, mat_R_ij, 100, 45, 0.95, 500, domain, log=True)
        if rank == 0:
            print("\nFinal value of x, F(x) is : ",xstar,",",F_xstar,"")
            print("time = ",time.time()-start)
